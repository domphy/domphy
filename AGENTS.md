...
| `@domphy/blocks` | 252 composed blocks/effect components — sidebar layouts, auth pages, a dashboard, `@domphy/chart` recipes (from shadcn/ui), and marketing/effect components (from Magic UI + Aceternity UI). Every export is a **factory function** `name(props?) => DomphyElement` returning a full mountable tree (not a `$`-patch). Clean-room reimplemented — see `packages/blocks/SOURCES.md` |
