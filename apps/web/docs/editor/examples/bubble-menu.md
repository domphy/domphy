...
- **The panel is portaled into a document/root overlay**, never a child of the contenteditable host (`EditorView.render()` wipes that element's children and would not put the toolbar back). It escapes the editor's overflow and stacking context, and it copies the nearest `[data-theme]` from the editor — floating content is a DOM sibling of the page, not of the editor, so it would otherwise resolve theme variables against the wrong scope. Destroying the editor hides and removes the toolbar.
