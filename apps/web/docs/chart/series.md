...
```ts
{
  calendar: {
    ...
    cellSize?: number | [w, h],           // default 20; "auto" per dimension fills the rect
...
Draws curved arcs between coordinate pairs on a geo map. Useful for migration flows, flight routes, or any directed relationship between locations.

Requires `geo` in the option and a registered map (see [Map / choropleth](#map-choropleth)).
...

{
  ...
  series: [{
    ...
    data: [
      { coords: [[-87.6, 41.8], [-118.2, 34.0]], name: "Chicago → LA" },
      { coords: [[-74.0, 40.7], [-122.4, 37.8]], name: "NYC → SF" },
    ],
    lineStyle: { width: 1.5, opacity: 0.7 },
    // Animated dot moving along each line:
    effect: {
      show: true,
      period: 3,      // seconds per traversal
      symbolSize: 6,
    },
    color: "primary",
  }],
...
Without `effect.show`, lines render as static bezier curves. With it, a dot animates along each arc using SVG `animateMotion`.

---

## EffectScatter

Scatter chart where each point has a ripple pulse animation. Good for highlighting locations on a geo map or drawing attention to specific data points.
...
// On a geo map:
{
  ...
  series: [{
    ...
    coordinateSystem: "geo",
    data: [
      [116.4, 39.9],   // [lng, lat] — Beijing
      [-74.0, 40.7],   // New York
      [2.3, 48.9],     // Paris
    ],
    symbolSize: 12,
    rippleEffect: {
      period: 2,       // seconds per ripple cycle
      scale: 4,        // ripple expands to 4× symbol size
      brushType: "fill",
    },
    color: "primary",
  }],
...
Bar chart where each bar is replaced by a repeating or scaled SVG symbol.
...
```ts
{
  ...
  series: [{
    type: "pictorialBar",
    symbol: "circle",      // "circle" | "rect" | "triangle" | "diamond" | "arrow" | "pin" | "path://..."
    symbolSize: [18, 18],  // [width, height]
    symbolRepeat: true,    // stack symbols to fill bar height
    symbolClip: true,      // clip last symbol to exact value
    data: [4, 7, 3, 9],
    color: "primary",
  }],
}
...
{
  type: "pictorialBar",
  symbol: "path://M0,-10 L3,-3 L10,-3 L4,2 L6,10 L0,5 L-6,10 L-4,2 L-10,-3 L-3,-3 Z",  // star
  symbolSize: [20, 20],
  symbolRepeat: 5,    // show exactly 5 symbols per bar, scale to bar height
  data: [3, 5, 2, 4],
}
```
