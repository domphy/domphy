...
These are upstream `@tanstack/form-core` files (byte-identical at `@tanstack/form-core@1.33.5` — see `packages/form/SOURCES.md`). Adapter-only additions live in `src/domphy/`. See `packages/form/tests/groups.test.ts` for the contract.
