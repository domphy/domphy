...
| `direction` | `BlurFadeDirection` | Direction the content slides in *from* — `"down"` (the default) starts the content offset below its final position and it slides upward into place; `"up"` starts above and slides down; `"left"`/`"right"` slide in from that side. |
