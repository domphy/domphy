...
|---|---|---|
| `children` | `DomphyElement \| DomphyElement[] \| string` | Content wrapped by the effect — typically a short word/phrase inside a heading. Defaults to `"Domphy"`. |
| `className` | `string` | Extra class name merged onto the text element. |
| `accentColor` | `ThemeColor` | Theme color family tinting the beams and edge glow lines. Defaults to `"info"`. |
| `alwaysOn` | `boolean` | Keeps the panel/beams/sparkles visible permanently instead of only on hover. Defaults to `false`. |
| `sparkleCount` | `number` | Roughly how many sparkles are alive at once while hovered. Defaults to `7`. |
| `beamCount` | `number` | Number of sweeping beam strips. Defaults to `4`. |
| `cornerRadius` | `number` | Panel corner radius, in `themeSpacing` units. Defaults to `3`. |
