...
const riverData: [string, number, string][] = [
  ...months.map((m, i) => [m, frontendValues[i], "Frontend"] as [string, number, string]),
  ...months.map((m, i) => [m, backendValues[i],  "Backend"]  as [string, number, string]),
  ...months.map((m, i) => [m, mobileValues[i],   "Mobile"]   as [string, number, string]),
  ...months.map((m, i) => [m, devopsValues[i],   "DevOps"]   as [string, number, string]),
];
