...
- **[`llms-full.txt`](https://domphy.com/llms-full.txt)** — one-shot full dump (~1.5 MB). Critical rules + quickstart + every core/theme doc + every patch source file. Use when you want the AI to have the whole framework in a single fetch.

`llms-full.txt` is generated by `pnpm llms` (`scripts/llms-full.mjs`) from AGENTS.md, DESIGN.md, docs, and patch source. `llms.txt` is curated by hand — that script does not write it.
...
5. **Package docs** — query, router, table, virtual, form, dnd, blocks, chart, three, editor, palette (part of `@domphy/theme`), app, doctor, i18n, press, mcp. The Markdown pipeline is named exports of `@domphy/press` (main entry, not `@domphy/markdown`). Mermaid is docs-site internal (`apps/web/mermaid/`), not a published `@domphy/*` package.
