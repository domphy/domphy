...
export function renderVisualMap(
  ...
  for (let vi = 0; vi < visualMaps.length; vi++) {
    ...
    const colors =
      (vm.inRange?.color as any as string[] | undefined) ?? DEFAULT_COLORS;
    const min = vm.min ?? 0;
    const max = vm.max ?? 100;
    const orient = vm.orient ?? "vertical";
    const itemW =
      orient === "vertical" ? (vm.itemWidth ?? 20) : (vm.itemHeight ?? 14);
    const itemH =
    ...
    if (vm.type === "piecewise") {
      ...
      for (let pi = 0; pi < pieces.length; pi++) {
        ...
        const color =
          (piece.color as string | undefined) ??
          interpolateColor(colors, pi / Math.max(1, pieces.length - 1));
        group.appendChild(
          svgEl("rect", {
            x,
            y: oy,
      ...
      if (orient === "vertical") {
        ...
        group.appendChild(
          svgText(String(min), x + itemW / 2, y + itemH + 12, {
            fill: textColor,
            "font-size": 10,
            "text-anchor": "middle",
          }),
        ...
        group.appendChild(
          svgText(String(min), x - 4, y + itemW / 2, {
            fill: textColor,
            "font-size": 10,
            "text-anchor": "end",
            "dominant-baseline": "middle",
        ...
        group.appendChild(
          svgText(String(max), x + itemH + 4, y + itemW / 2, {
            fill: textColor,
            "font-size": 10,
            "text-anchor": "start",
            "dominant-baseline": "middle",
      ...
      if (vm.text) {
        ...
        if (orient === "vertical") {
          if (maxLabel)
            group.appendChild(
              svgText(maxLabel, x + itemW / 2, y - 18, {
                fill: textColor,
                "font-size": 11,
                "text-anchor": "middle",
              }),
          ...
          if (minLabel)
            group.appendChild(
              svgText(minLabel, x + itemW / 2, y + itemH + 28, {
                fill: textColor,
                "font-size": 11,
                "text-anchor": "middle",
              }),
