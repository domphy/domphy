import type { LinesSeriesOption, GeoOption } from "../types.js";
import { seriesHex } from "../gl/color.js";
import { getRegisteredMap } from "./geomap.js";

function svgEl(tag: string, attrs: Record<string, string | number | boolean>): SVGElement {
...
function buildViewport(geo: GeoOption, geoJSON: any, width: number, height: number): GeoViewport | null {
  ...
  for (const feature of (geoJSON?.features ?? [])) {
    ...
    if (!geom) continue;
    const depth = geom.type === "Polygon" ? 2 : geom.type === "MultiPolygon" ? 3 : geom.type === "LineString" ? 1 : geom.type === "MultiLineString" ? 2 : 0;
...
function lngLatToPixel(lng: number, lat: number, vp: GeoViewport): [number, number] {
  const [nx, ny] = mercatorProject(lng, lat);
  const px = vp.left + (nx - vp.minX) * vp.scaleX + (vp.w - (vp.scaleX * /* spanX approx */ 1)) / 2;
  const py = vp.top + (ny - vp.minY) * vp.scaleY + (vp.h - (vp.scaleY * 1)) / 2;
  return [px, py];
}

// Better: use same logic as geomap viewport — compute centroid offset
function lngLatToPixelVp(lng: number, lat: number, vp: GeoViewport, centerOffsetX: number, centerOffsetY: number): [number, number] {
  const [nx, ny] = mercatorProject(lng, lat);
  return [
...
export function renderLines(
  ...
  const geo = geos[0];
  const geoName = geo?.map ?? linesSeries[0]?.geoIndex !== undefined ? "" : "";
  const mapName = geo?.map ?? "world";
  const geoJSON = getRegisteredMap(mapName);
  const vp = geoJSON ? buildViewport(geo ?? { map: mapName }, geoJSON, width, height) : null;
    ...
    for (const f of (geoJSON.features ?? [])) {
      ...
      function walkCoords(c: any, d: number): void {
        ...
        if (d === 0 || (d === 1 && typeof c[0] === "number")) {
          ...
          if (px < minX) minX = px; if (px > maxX) maxX = px;
          if (py < minY) minY = py; if (py > maxY) maxY = py;
        } else { for (const x of c) walkCoords(x, d - 1); }
      }
      const depth = geom.type === "Polygon" ? 2 : geom.type === "MultiPolygon" ? 3 : geom.type === "LineString" ? 1 : geom.type === "MultiLineString" ? 2 : 0;
  ...
  for (let si = 0; si < linesSeries.length; si++) {
    ...
    for (const item of (s.data ?? [])) {
      ...
      if (s.coordinateSystem === "geo" || !s.coordinateSystem) {
        if (!vp) continue;
        [x1, y1] = lngLatToPixelVp(from[0], from[1], vp, centerOffsetX, centerOffsetY);
        [x2, y2] = lngLatToPixelVp(to[0], to[1], vp, centerOffsetX, centerOffsetY);
      } else {
