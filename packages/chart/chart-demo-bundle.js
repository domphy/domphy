...
// src/overlay/graph.ts
function nodeId(n, index) {
  return n.id ?? n.name ?? String(index);
}
function runForce(nodes, edges, width, height, iterations, repulsion, gravity, idealLength) {
  const cx = width / 2;
  const cy = height / 2;
...
function renderGraph(svg, series, width, height, hiddenSeries) {
  ...
  for (let si = 0; si < series.length; si++) {
    ...
    const nodeMap = /* @__PURE__ */ new Map();
    const physicsNodes = rawNodes.map((n, index) => {
      const id = nodeId(n, index);
      const catIndex = n.category ?? si % 9;
      const color = n.color ? seriesHex(catIndex) : categories[catIndex]?.color ? seriesHex(catIndex) : seriesHex(catIndex);
      const radius = n.symbolSize ? (Array.isArray(n.symbolSize) ? n.symbolSize[0] : n.symbolSize) / 2 : defaultRadius;
      const angle = index / rawNodes.length * Math.PI * 2;
      const initR = Math.min(areaW, areaH) * 0.35;
      const initX = n.x !== void 0 ? left + n.x * areaW : left + areaW / 2 + Math.cos(angle) * initR;
      const initY = n.y !== void 0 ? top + n.y * areaH : top + areaH / 2 + Math.sin(angle) * initR;
      const node = {
        id,
        x: initX,
        y: initY,
    ...
    if (layout === "force") {
      ...
      const repulsion = forceOpts.repulsion ?? 100;
      const gravity = forceOpts.gravity ?? 0.1;
      const edgeLength = Array.isArray(forceOpts.edgeLength) ? forceOpts.edgeLength[0] : forceOpts.edgeLength ?? 80;
    ...
    for (const node of physicsNodes) {
      ...
      if (node.label && node.radius >= 6) {
        ...
        text2.setAttribute("x", String(node.x));
        text2.setAttribute("y", String(node.y + node.radius + 11));
        text2.setAttribute("text-anchor", "middle");
        text2.setAttribute("font-size", "10");
        text2.setAttribute("fill", "#555");
        text2.setAttribute("pointer-events", "none");
        nodesGroup.appendChild(text2);
      }
    }
...
var ChartEngine = class {
  ...
  render() {
    ...
    const graphSeries = series.filter((s) => s.type === "graph");
