...
await mount(card("Line chart — smooth + area"), {
  ...
  series: [
    ...
    },
    {
      ...
      smooth: true,
      data: [
        900, 1050, 1020, 1100, 1400, 1500, 1450, 1000, 1300, 1350, 1000, 1100,
      ],
    },
  ],
});
...
for (let d = 0; d < 7; d++)
  for (let h = 0; h < 24; h++)
    heatData.push([h, d, Math.floor(Math.random() * 10)]);
...
// ─── DataZoom ─────────────────────────────────────────────────────────────────
const dzData = Array.from({ length: 100 }, (_, i) => [
  i,
  Math.sin(i * 0.2) * 50 + Math.random() * 20 + 50,
]);
