// @vitest-environment jsdom

import type { DomphyElement } from "@domphy/core";
import { ElementNode } from "@domphy/core";
import { afterEach, describe, expect, it } from "vitest";
import { chartLineStep } from "../../../src/shadcn/charts/chart-line-step.ts";

if (!("ResizeObserver" in globalThis)) {
  (globalThis as any).ResizeObserver = class {
    observe() {}
    unobserve() {}
    disconnect() {}
  };
}

function render(app: DomphyElement) {
  const host = document.createElement("div");
  document.body.appendChild(host);
  const node = new ElementNode(app);
  node.render(host);
  return { host, node };
}

afterEach(() => {
  document.body.innerHTML = "";
});

describe("chartLineStep", () => {
  it("renders a working demo tree with zero args: titled card, chart plot, trend footer", () => {
    const { host } = render(chartLineStep() as DomphyElement);
    expect(host.querySelector("h3")?.textContent).toBe("Line Chart - Step");
    expect(host.querySelector("footer")).toBeTruthy();
    expect(host.querySelector("canvas")).toBeTruthy();
  });

  it("accepts a custom stepMode prop", () => {
    const { host } = render(chartLineStep({ stepMode: "end" }) as DomphyElement);
    expect(host.querySelector("canvas")).toBeTruthy();
  });
});
