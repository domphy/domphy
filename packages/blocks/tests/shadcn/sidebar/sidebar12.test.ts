...
describe("sidebar12", () => {
  ...
  it("renders one checkbox per calendar entry, only the first two per group checked by default", () => {
    const { host } = render(sidebar12() as DomphyElement);
    const checkboxes = Array.from(host.querySelectorAll('aside input[type=checkbox]')) as HTMLInputElement[];
  ...
  it("clicking a calendar checkbox toggles it off", () => {
    const { host } = render(sidebar12() as DomphyElement);
    const checkbox = host.querySelector('aside input[type=checkbox]') as HTMLInputElement;
  ...
  it("month-picker prev/next arrows change the displayed month label", async () => {
    const { host } = render(sidebar12() as DomphyElement);
    const nextButton = host.querySelector('aside button[aria-label="Next month"]') as HTMLButtonElement;
  ...
  it("clicking a day cell selects it without throwing", () => {
    const { host } = render(sidebar12() as DomphyElement);
    const dayButtons = Array.from(host.querySelectorAll("aside button")).filter((button) => /^\d+$/.test(button.textContent ?? ""));
  ...
  it("accepts custom user and calendar groups", () => {
    const { host } = render(
      sidebar12({
        ...
        groups: [{ label: "Solo", entries: [{ id: "x", name: "Custom Calendar", color: "primary" }] }],
      }) as DomphyElement,
    );
