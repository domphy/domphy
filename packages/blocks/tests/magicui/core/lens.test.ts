...
describe("lens", () => {
  it("renders a working demo tree with zero args: base content plus a hidden circular overlay with a cloned zoom layer", () => {
    ...
    const overlay = wrapper.querySelector(
      '[data-lens-overlay="true"]',
    ) as HTMLElement;
    expect(overlay).toBeTruthy();

    const zoomLayer = overlay.querySelector(
      '[data-lens-zoom-layer="true"]',
    ) as HTMLElement;
  ...
  it("tracks mousemove and fades the overlay in/out on enter/leave (follow mode)", () => {
    ...
    wrapper.dispatchEvent(
      new MouseEvent("mouseenter", { clientX: 40, clientY: 30 }),
    );
  ...
  it("pins the lens visible immediately and reacts to position state updates in static mode", () => {
    ...
    const { host } = render(
      lens({ isStatic: true, position: positionState }) as DomphyElement,
    );
    const wrapper = host.firstElementChild as HTMLElement;
    const overlay = wrapper.querySelector(
      '[data-lens-overlay="true"]',
