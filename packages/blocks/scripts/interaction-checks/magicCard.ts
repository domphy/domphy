...
async function main() {
  ...
  try {
    ...
    if (!box) {
      ...
      const opacityA = await glowLayer.evaluate((el) => (el as HTMLElement).style.opacity);

      await page.mouse.move(box.x + box.width * 0.8, box.y + box.height * 0.8, { steps: 5 });
      await page.waitForTimeout(100);
      const positionB = await readVars();
      const opacityB = await glowLayer.evaluate((el) => (el as HTMLElement).style.opacity);
      ...
      report("magicCard glow is visible while hovering", opacityA === "1" && opacityB === "1", `opacityA=${opacityA} opacityB=${opacityB}`);

      await page.mouse.move(box.x - 100, box.y - 100, { steps: 5 });
      await page.waitForTimeout(300);
      const opacityAfterLeave = await glowLayer.evaluate((el) => (el as HTMLElement).style.opacity);
      report("magicCard glow fades out on mouse-leave", opacityAfterLeave === "0", `opacity=${opacityAfterLeave}`);
    }
