// Real-browser interaction check for chartBarInteractive: hovering the bars
// shows a real per-point tooltip, and clicking the other header stat tile
...
async function main() {
  ...
  report(
    "chartBarInteractive:hover-shows-per-point-tooltip",
    atLeftEdge?.opacity === "1" &&
      atRightEdge?.opacity === "1" &&
      /\d/.test(atLeftEdge?.text ?? "") &&
      atLeftEdge?.text !== atRightEdge?.text,
    `expected a visible tooltip with a numeric value, DIFFERENT at the two plot edges (proving it tracks the hovered day); left=${JSON.stringify(atLeftEdge)} right=${JSON.stringify(atRightEdge)}`,
  );

  // Series switch: clicking the "Mobile" stat tile should flip data-active
  // on both tiles and re-render different bars.
  ...
  report(
    "chartBarInteractive:series-switch-changes-rendered-bars",
    !beforeSwitch.equals(afterSwitch),
    `expected the rendered bar pixels to differ after switching from Desktop to Mobile series (${beforeSwitch.length} vs ${afterSwitch.length} bytes)`,
  );
...
main()
  ...
  .catch((error) => {
    console.error(error);
    report("chartBarInteractive:script-error", false, String(error));
    summarize();
  });
