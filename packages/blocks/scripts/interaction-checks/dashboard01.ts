...
async function main(): Promise<void> {
  ...
  try {
    ...
    const captionBefore = await block.locator("text=Showing total visitors for the").innerText();
    await block.locator('button[role="button"]', { hasText: "Last 7 days" }).click();
    await page.waitForTimeout(200);
    const captionAfter = await block.locator("text=Showing total visitors for the").innerText();
    report(
      "dashboard01: switching the date-range control re-renders the chart with different data",
      captionBefore.includes("3 months") && captionAfter.includes("7 days") && captionBefore !== captionAfter,
      `caption before="${captionBefore}" after="${captionAfter}"`,
    );

    // The toggle-group's pressed state should also move to the clicked item.
    const sevenDaysButton = block.locator('button[role="button"]', { hasText: "Last 7 days" });
    const threeMonthsButton = block.locator('button[role="button"]', { hasText: "Last 3 months" });
    const sevenPressed = await sevenDaysButton.getAttribute("aria-pressed");
    const threeMonthsPressed = await threeMonthsButton.getAttribute("aria-pressed");
    report(
