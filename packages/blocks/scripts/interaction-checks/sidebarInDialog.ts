...
async function main(): Promise<void> {
  ...
  try {
    ...
    const openBefore = await dialogElement.evaluate((node) => (node as HTMLDialogElement).open);
    const visibleBefore = await dialogElement.isVisible();

    await block.locator("button", { hasText: "Open settings" }).click();
    await page.waitForTimeout(250);

    const openAfter = await dialogElement.evaluate((node) => (node as HTMLDialogElement).open);
    ...
    report(
      ...
      openBefore === false && !visibleBefore && openAfter === true && visibleAfter && focusInsideDialog,
    ...
    const categoryButton = dialogElement.locator("button", { hasText: "Appearance" });
    await categoryButton.click();
    await page.waitForTimeout(100);
    const currentCrumb = await dialogElement.locator('[aria-current="page"]').innerText();
    report(
    ...
    const openAfterEscape = await dialogElement.evaluate((node) => (node as HTMLDialogElement).open);
