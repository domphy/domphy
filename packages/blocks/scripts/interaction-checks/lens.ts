...
async function main() {
  ...
  const baseContent = page.locator(
    '[data-block="lens"] [data-lens-content="true"]',
  );
  const overlay = page.locator(
    '[data-block="lens"] [data-lens-overlay="true"]',
  );
  const box = await baseContent.boundingBox();
  if (!box) throw new Error("lens base content has no bounding box");
  const lensRadius = await overlay.evaluate(
    (el) => (el as HTMLElement).offsetWidth / 2,
  );

  const opacityOutside = await overlay.evaluate(
  ...
  const transformB = await overlay.evaluate(
  ...
  const expectedA = {
    x: pointA.x - box.x - lensRadius,
    y: pointA.y - box.y - lensRadius,
  };
  const expectedB = {
    x: pointB.x - box.x - lensRadius,
    y: pointB.y - box.y - lensRadius,
  };
  ...
  const matchesA =
    !!resultA &&
    withinTolerance(resultA.x, expectedA.x, 2) &&
    withinTolerance(resultA.y, expectedA.y, 2);
  const matchesB =
    !!resultB &&
    withinTolerance(resultB.x, expectedB.x, 2) &&
    withinTolerance(resultB.y, expectedB.y, 2);
  ...
  report(
    ...
    matchesA &&
      ...
      !!resultA &&
      !!resultB &&
      (resultA.x !== resultB.x || resultA.y !== resultB.y),
