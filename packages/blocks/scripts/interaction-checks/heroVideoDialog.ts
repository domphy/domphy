...
async function main(): Promise<void> {
  ...
  try {
    ...
    const playButton = block.locator('[role="button"][aria-label^="Play video"]');
    ...
    const iframeSrcWhileOpen = await dialogLocator.locator("iframe").getAttribute("src");
    report(
    ...
    const iframeSrcAfterClose = await dialogLocator.locator("iframe").getAttribute("src");
    report(
