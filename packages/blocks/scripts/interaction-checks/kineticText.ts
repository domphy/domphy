...
async function main() {
  ...
  if (letterCount < 10) throw new Error(`expected >=10 character spans, got ${letterCount}`);
  ...
  const weightBeforeHover = await letters.nth(hoveredIndex).evaluate((element) => getComputedStyle(element).fontWeight);
  ...
  const weightWhileHovering = await letters.nth(hoveredIndex).evaluate((element) => getComputedStyle(element).fontWeight);
  const farWeightWhileHovering = await letters.nth(farIndex).evaluate((element) => getComputedStyle(element).fontWeight);

  report(
    "kineticText: hovered letter's font-weight actually increases above baseline",
    Number(weightBeforeHover) === Number(BASE_WEIGHT) && Number(weightWhileHovering) > Number(BASE_WEIGHT),
  ...
  const weightAfterLeave = await letters.nth(hoveredIndex).evaluate((element) => getComputedStyle(element).fontWeight);
  report(
