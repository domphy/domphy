...
function mount(name: string, factory: () => DomphyElement, box: HTMLElement): void {
  box.textContent = "";
  try {
    new ElementNode(factory()).render(box);
  } catch (error) {
...
(window as unknown as { mountBlock: (name: string) => void }).mountBlock = (name: string) => {
  if (mounted.has(name)) return;
  const factory = blocks[name as keyof typeof blocks] as unknown as (() => DomphyElement) | undefined;
