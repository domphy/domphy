...
function indexText(
  ...
  entryIndex: number, text: string, fieldWeight: number,
): void {
  for (const term of tokenize(text)) {
    let perEntry = postings.get(term)
    if (!perEntry) { perEntry = new Map(); postings.set(term, perEntry) }
    const existing = perEntry.get(entryIndex)
    if (existing === undefined || fieldWeight > existing) perEntry.set(entryIndex, fieldWeight)
  }
}
...
export function buildSearchIndex(docs: SearchDocument[]): string {
  ...
  return JSON.stringify({ entries, postings: serializedPostings } satisfies SerializedIndex)
}

// --- Query -------------------------------------------------------------------

export function queryIndex(serializedIndex: string, query: string, limit = 10): SearchResult[] {
  ...
  for (const term of terms) {
    ...
    for (const indexedTerm in index.postings) {
      ...
      for (const [entryIndex, fieldWeight] of index.postings[indexedTerm]) {
        ...
        scoreByEntry.set(entryIndex, (scoreByEntry.get(entryIndex) ?? 0) + contribution)
        if (!seen.has(entryIndex)) {
          seen.add(entryIndex)
          matchedTermsByEntry.set(entryIndex, (matchedTermsByEntry.get(entryIndex) ?? 0) + 1)
        }
      }
    }
  }
  ...
  for (const [entryIndex, score] of scoreByEntry) {
    ...
    results.push({
      route: entry.route, pageTitle: entry.pageTitle, heading: entry.heading,
      slug: entry.slug, isPage: entry.isPage,
...
function resultRow(
  result: SearchResult, resultIndex: number, widgetId: number,
  ...
  return {
    a: [
      ...
      result.isPage ? null : { small: result.pageTitle, $: [small({ color: "neutral" })] },
    ],
    href: result.href, role: "option", id,
    ariaSelected: (l) => state.get("active", l) === resultIndex || undefined,
    onClick: () => { state.set("open", false) },
    onMouseEnter: () => { state.set("active", resultIndex) },
    $: [link({ color: "neutral", accentColor: "primary" })],
    style: {
      display: "flex", flexDirection: "column", gap: themeSpacing(1),
      paddingBlock: themeSpacing(2), paddingInline: themeSpacing(3),
...
export function searchWidget(options: SearchWidgetOptions = {}): DomphyElement {
  const { indexUrl = "/search-index.json", placeholder = "Search docs…", limit = 10 } = options
  const widgetId = ++widgetCounter
  const listboxId = `dp-search-${widgetId}-listbox`

  const state = new RecordState<WidgetState>({ index: null, query: "", results: [], open: false, active: -1 })
  ...
  const input: DomphyElement = {
    input: null, type: "search", placeholder, autocomplete: "off",
    role: "combobox",
    ariaExpanded: (l) => (state.get("open", l) ? "true" : "false"),
    ariaControls: listboxId, ariaAutocomplete: "list",
    ariaActiveDescendant: (l) => { const a = state.get("active", l); return a >= 0 ? optionId(widgetId, a) : undefined },
    ...
    onFocus: () => { if (state.get("results").length > 0) state.set("open", true) },
    onKeyDown: (e: KeyboardEvent) => {
      const results = state.get("results")
      if (e.key === "Escape") { state.set("open", false); state.set("active", -1); return }
      if (e.key === "ArrowDown") {
        if (!results.length) return; e.preventDefault()
        state.set("open", true); state.set("active", (state.get("active") + 1) % results.length); return
      }
      if (e.key === "ArrowUp") {
        if (!results.length) return; e.preventDefault()
        state.set("active", (state.get("active") - 1 + results.length) % results.length); return
      }
      if (e.key === "Enter") {
        const active = state.get("active"); navigate(active >= 0 ? results[active] : results[0])
      }
    },
  ...
  const dropdown: DomphyElement = {
    div: (l) => state.get("open", l)
      ? state.get("results", l).map((r, i) => resultRow(r, i, widgetId, state))
      : [],
    ...
    style: {
      ...
      marginBlockStart: themeSpacing(1), maxHeight: themeSpacing(80),
  ...
  return {
    ...
    _onMount: (node) => {
      fetch(indexUrl).then(r => r.text()).then(text => {
        state.set("index", text)
        if (state.get("query").trim()) runQuery(state, limit)
      }).catch(() => {})
      const host = node.domElement as HTMLElement
      const handler = (e: Event) => { if (!host.contains(e.target as Node)) state.set("open", false) }
      document.addEventListener("pointerdown", handler)
      node.setMetadata("dpSearchHandler", handler)
    },
    _onRemove: (node) => {
      const handler = node.getMetadata("dpSearchHandler") as ((e: Event) => void) | undefined
...
export function mountSearch(host: HTMLElement, options: SearchWidgetOptions = {}): ElementNode {
