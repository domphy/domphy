...
export type { AuditIssue, AuditOptions, AuditPage, AuditResult, Rect } from "./types.js";
...
export async function checkLayout(page: AuditPage, options: AuditOptions = {}): Promise<AuditResult> {
  const checks = options.checks ?? ["overlap", "geometry", "contrast"];

  const [overlapIssues, geometryIssues, contrastIssues, layout] = await Promise.all([
    checks.includes("overlap") ? detectOverlaps(page) : Promise.resolve([]),
    checks.includes("geometry") ? verifyGeometry(page, options.tolerance ?? 1) : Promise.resolve([]),
    checks.includes("contrast") ? checkContrast(page, options.minContrast ?? 4.5) : Promise.resolve([]),
    snapshot(page),
  ]);
