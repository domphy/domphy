...
export class Ramp {
  ...
  get baseIndex() {
    ...
    return this.colors.findIndex(
      (hex) => hex.toLowerCase() === this.baseColor.toLowerCase(),
    );
  }
  ...
  get apca(): ApcaContrasts {
    ...
    const apcaContrast = (yText: number, yBg: number) => {
      ...
      let lc =
        bg >= txt
          ? (bg ** 0.56 - txt ** 0.57) * 114
          : (bg ** 0.65 - txt ** 0.62) * 114;
    ...
    for (const level of [45, 60, 75] as const) {
      ...
      for (let k = 1; k < total; k++) {
        ...
        for (let i = 0; i < total - k; i++) {
          const bg =
            swatches[i].luminance > swatches[i + k].luminance
              ? swatches[i].luminance
              : swatches[i + k].luminance;
          const text =
            swatches[i].luminance > swatches[i + k].luminance
              ? swatches[i + k].luminance
              : swatches[i].luminance;
  ...
  get deltaECurve() {
    ...
    for (let i = 1; i < this.swatches.length; i++) {
      const de00 = calcDeltaE2000(
        this.swatches[i - 1].lab,
        this.swatches[i].lab,
      );
  ...
  get lightnessLinearity() {
    ...
    return Math.max(
      0,
      Math.min(1, 1 - Math.sqrt(sumSqError / n) / Math.sqrt(sumSqMaxError / n)),
    );
  }
  ...
  get chromaSmoothness() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      sumSqErr += err * err;
      sumSqMaxErr += Math.max(target - cMin, cMax - target) ** 2;
    }

    return Math.max(
      0,
      Math.min(1, 1 - Math.sqrt(sumSqErr / n) / Math.sqrt(sumSqMaxErr / n)),
    );
  }
  ...
  get spacingUniformity() {
    ...
    let sumSq = 0;
    for (const d of deltas) sumSq += (d - mean) ** 2;
  ...
  get hueStability() {
    ...
    const ref =
      values[this.baseIndex - 1] ?? this.swatches[this.baseIndex]?.hue ?? 0;
    ...
    return Math.max(
      ...
      Math.min(
        1,
        1 - Math.sqrt(sumSqError / n) / (Math.sqrt(sumSqMaxError / n) || 1),
      ),
    );
  }
