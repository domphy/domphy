import { type ApcaContrasts, Ramp, type WcagContrasts } from "./Ramp.js";
...
export class Palette {
  ...
  constructor(colors: PaletteColors = {}, name = "Collection 1") {
    this.ramps = Object.entries(colors).map(
      ([rampName, rampColors]) => new Ramp(rampColors, rampName),
    );
    this.name = name;
  }
  ...
  get colors(): PaletteColors {
    return Object.fromEntries(
      this.ramps.map((ramp) => [ramp.name, ramp.colors]),
    );
  }
  ...
  get wcag(): WcagContrasts {
    ...
    for (const level of [30, 45, 70] as const) {
      ...
      const span = Math.max(
        0,
        ...rampContrasts.map((contrast) => contrast?.span || 0),
      );
      const sum = rampContrasts.reduce(
        (acc, contrast) => acc + (contrast?.value || 0),
        0,
      );
