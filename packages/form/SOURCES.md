...
| 7 | `src/FormApi.ts` (`deleteField`) | Sub-field match uses `isFieldInGroup` (`.` / `[` path boundary) instead of bare `startsWith(fieldStr)`. | Bare `startsWith` treats `emailConfirm` as a child of `email` and `tags.10` as a child of `tags.1`. Bracket form `tags[1]` vs `tags[10]` is already safe because `]` terminates the index, but the dotted form and prefix-sharing names are not. Regression: `tests/array-fields.test.ts` ("FormApi.deleteField: path-boundary"). **Still present upstream in 1.33.3.** |
| 8 | `src/FormApi.ts` (`_handleSubmit`) | Early-return when `state.isSubmitting` is already true (devtools override still bypasses). | `!canSubmit` only early-returns when `submissionAttempts <= 1`. An overlapping `handleSubmit` while the first run is in flight increments attempts to 2, skips that return, and calls `onSubmit` again. Sequential re-submit after the previous run finishes is unchanged (stale-error revalidation). Regression: `tests/adapter-async-submit.test.ts` ("a second overlapping handleSubmit does not run onSubmit"). **Still present upstream in 1.33.3.** |
...
- `pnpm --filter @domphy/form test` — 125 tests across 9 files (headless core suites + jsdom adapter lifecycle suite), including the flipped re-submission test, the deviation #4 regression test, `deleteField` path-boundary tests (deviation #7), and the overlapping-submit guard (deviation #8).
